...
//}
  ...
  // */
  // void FastTrackerMPCUDA::parallelElementWiseMult(Mat & src, const float scalar, const int batch_size) {
  ...
  void inline FastTrackerMPCUDA::fft2(const Mat src, Mat & dest) const {
    ...
    if (!use_cuda){
      ...
      cuda::dft(d_src, d_dest, src.size(), DFT_SCALE);
  ...
  void inline FastTrackerMPCUDA::fft2(const Mat src, std::vector<Mat> & dest, std::vector<Mat> & layers_data) const {
    ...
    for(int i=0; i < src.channels(); i++){
      ...
      // cuda::dft(s, d, layers_data[i].size(), DFT_ROWS);
      // d.download(dest[i]);
