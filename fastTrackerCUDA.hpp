...
public:
    struct Params {
        ...
        int desc_pca;        //!<  compressed descriptors of FastTrackerCUDA::MODE
        int desc_npca;       //!<  non-compressed descriptors of FastTrackerCUDA::MODE
...
private:
    ...
    bool getSubWindow(const Mat img, const Rect roi, Mat& feat, Mat& patch, FastTrackerCUDA::MODE desc = GRAY) const;
...
public:
    FastTrackerCUDA(FastTrackerCUDA::Params p);  // use ::create()
    
    Params params;

    static Ptr<FastTrackerCUDA> create() {
        FastTrackerCUDA::Params p = FastTrackerCUDA::Params{};
        return makePtr<FastTrackerCUDA>(p);
    }
